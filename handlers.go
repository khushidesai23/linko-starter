...
func (s *server) handlerShortenLink(w http.ResponseWriter, r *http.Request) {
	...
	if s.stdLogger != nil {
		s.stdLogger.Println("Shortening URL:", longURL)
	}
	...
	if s.stdLogger != nil {
		s.stdLogger.Printf("Parsed URL: scheme=%s, host=%s", u.Scheme, u.Host)
	}
	...
	if s.stdLogger != nil {
		s.stdLogger.Printf("Generated short code: %s for URL: %s", shortCode, longURL)
	}
...
func (s *server) handlerRedirect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			...
			if s.stdLogger != nil {
				s.stdLogger.Printf("failed to lookup URL: %v", err)
			}
...
func (s *server) handlerListURLs(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if s.stdLogger != nil {
			s.stdLogger.Printf("failed to list URLs: %v", err)
		}
